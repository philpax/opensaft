...
impl IsoTransform {
    ...
    pub fn look_at_rh(eye: Vec3, target: Vec3, up: Vec3) -> Option<Self> {
        let rotation = rotate_negative_z_towards(target - eye, up)?;
...
fn rotate_negative_z_towards(forward: Vec3, up: Vec3) -> Option<Quat> {
    ...
    if forward != Vec3::ZERO && side != Vec3::ZERO && up != Vec3::ZERO {
        Some(Quat::from_mat3(&glam::Mat3::from_cols(side, up, -forward)))
    } else {
        None
    }
}
...
mod test {
    ...
    fn test_rotate_negative_z_towards() {
        ...
        let rot = rotate_negative_z_towards(desired_fwd, desired_up).unwrap();
