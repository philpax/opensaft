...
//!
//! Only contains the types that are strictly necessary for `opensaft`.

use glam::{Mat3A, Vec3};

/// A ray in 3-dimensional space: a line through space with a starting point and a direction.
///
/// Any point on the,ound through the formula `origin + t * dir`,
...
impl BoundingBox {
    ...
    #[must_use]
    #[cfg(not(target_arch = "spirv"))]
    pub fn rotated_around_origin(&self, q: &glam::Quat) -> Self {
        if self.is_nothing() {
            Self::nothing()
        } else {
...
#[cfg(not(target_arch = "spirv"))]
fn rotate_bounding_box(half_size: Vec3, center: Vec3, q: glam::Quat) -> BoundingBox {
    // Inspired by:
    // https://zeux.io/2010/10/17/aabb-from-obb-with-component-wise-abs
